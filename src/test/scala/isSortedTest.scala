...
class isSortedTest extends org.scalatest.TestSuite {
  ...
  // hashcode of c is 99
  // and so on
  assert(isSorted(Array("one", "two"), (x: String, y: String) => x(0).hashCode() < y(0).hashCode()))
  assert(isSorted(Array("abba", "baba"), (x: String, y: String) => x(0).hashCode() < y(0).hashCode()))
  assert(isSorted(Array("xerxes", "abba"), (x: String, y: String) => x(0).hashCode() > y(0).hashCode()))
}
